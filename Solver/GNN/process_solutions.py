...
for instance in instances:
    ...
    for estimator in other_cost_estimators:
        try:
            ...
            costs[estimator] = float(json_data['mse-test-set'])
    ...
    try:
        ...
        data = {'InstanceName' : instance, 
                'GNN': math.sqrt(float(float(jsonData['test-loss']))),
                'BD':  math.sqrt(float(costs['BD'])),
                'SNN': math.sqrt(float(costs['SNN'])),
                'FIG': math.sqrt(float(costs['FIG']))
        }
    except Exception:
...
df = df.sort_values(by=['Central Depot', 'Size', 'Target Size District', 'City', 'Depot'])
...
df_grouped.to_csv('all_median.csv', sep='\t')

# df.to_csv('complete_rmse.csv', sep='\t')